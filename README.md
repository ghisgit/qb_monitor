...
> 自动监控 qBittorrent 的标签驱动工具：按分类打标签、清理无用文件、卡顿降级，并用 DeepSeek Harness 识别媒体并匹配 TMDB，按 Jellyfin 结构硬链接整理入库。

基于标签驱动的事件处理架构：自动跳过不需要的文件、清理已完成种子的冗余数据、监控卡顿种子进行优先级降级；可选启用 AI 整理，把下载完成的媒体自动硬链接进 Jellyfin 结构化媒体库。
