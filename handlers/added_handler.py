...
class AddedHandler(BaseHandler):
    def handle(self, task: TorrentTask) -> None:
        ...
        # 跳过空种子或单文件种子（无选择必要）
        if not task.files or len(task.files) <= 1:
            self.logger.debug(
        ...
        self.logger.debug(
            f"    → Inspecting {len(task.files)} files against skip rules..."
        )

        # 构建 id → name 映射，避免后续重复遍历
        file_id_to_name = {f.id: f.name for f in task.files}

        files_to_disable: List[int] = [
            f.id for f in task.files if f.priority != 0 and self._match_rule(f.name)
        ]
        ...
        try:
            ...
            self.logger.info(
                f"    ✅ Skipped {len(files_to_disable)}/{len(task.files)} files "
                f"for '{task.name}'"
            )
            # DEBUG: 列出被跳过的文件名
